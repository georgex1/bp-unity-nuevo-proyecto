...
class pagination {
	...
	function pagination($full_sql, $per_page, $cur_page) {
		...
		$this->total = mysql_num_rows($rsdt);
		$this->tot_pages = ceil($this->total/$per_page);
	}
	...
	function get_links($display=10) {
		...
		if ($this->cur_page > 1) {
			...
			$page_link .= " <a href='".$link."page=$prev'>previo</a> ";
		}
		else {
            //$page_link .= "<span> previo</span>";
			$page_link .= "";
		}
		...
		for($i = $start; $i <= $end; $i++) {
			if ($i == $this->cur_page)
				$page_link .= "<span class='current'>$i</span>";
			else
		...
		if ($this->cur_page < $this->tot_pages) {
			...
			$page_link .= " <a href='".$link."page=$next'>siguiente</a> ";
		}
		else {
